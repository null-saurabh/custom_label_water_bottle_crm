...
class DashboardController extends GetxController {
  ...
  final weekDelivered =
      0.obs; // distinct orders delivered (any delivery entry) in ordersWeek
  ...
  int get maxWeeklyTotal => weeklyBars.fold<int>(
    0,
    (m, e) => (e.delivered + e.scheduled) > m ? (e.delivered + e.scheduled) : m,
  );
  ...
  void prevDueWeek() => selectedDueWeekAnchor.value = selectedDueWeekAnchor
      .value
      .subtract(const Duration(days: 7));
  void nextDueWeek() => selectedDueWeekAnchor.value = selectedDueWeekAnchor
      .value
      .add(const Duration(days: 7));

  void prevOrdersWeek() => selectedOrdersWeekAnchor.value =
      selectedOrdersWeekAnchor.value.subtract(const Duration(days: 7));
  void nextOrdersWeek() => selectedOrdersWeekAnchor.value =
      selectedOrdersWeekAnchor.value.add(const Duration(days: 7));
  ...
  final _ordersDueOrdersWeek =
      <OrderModel>[].obs; // for OrdersThisWeek scheduled
  ...
  void onInit() {
    ...
    _pagingWorker = everAll([
      selectedDay,
      selectedDueWeekAnchor,
      selectedOrdersWeekAnchor,
    ], (_) => _bindStreams());
  ...
  void _recompute({
    ...
    final dueTodayOrders =
        _ordersDueDay.where((o) {
          ...
          (a, b) => (_dueDateOf(a) ?? DateTime(2100)).compareTo(
            _dueDateOf(b) ?? DateTime(2100),
          ),
        );
  ...
  List<WeeklyBarData> _computeWeeklyBars(
    DateSpan week,
    List<OrderModel> scheduledOrdersWeek,
  ) {
    ...
    return days.map((d) {
      ...
      final scheduled = scheduledOrdersWeek
          .where((o) => _inSpan(_dueDateOf(o), daySpan))
          .length;
  ...
  DateTime? _dueDateOf(OrderModel o) =>
      o.nextDeliveryDate ?? o.expectedDeliveryDate;
  ...
  ChipStyle _weekdayChipStyle(String weekday) {
    switch (weekday) {
      case 'Monday':
        return ChipStyle(const Color(0xFFE8F5E9), const Color(0xFF388E3C));
      case 'Tuesday':
        return ChipStyle(const Color(0xFFE3F2FD), const Color(0xFF1E88E5));
      case 'Wednesday':
        return ChipStyle(const Color(0xFFE0F2F1), const Color(0xFF00796B));
      case 'Thursday':
        return ChipStyle(const Color(0xFFFFF3E0), const Color(0xFFF57C00));
      default:
        return ChipStyle(const Color(0xFFF1F4FA), const Color(0xFF374151));
    }
  }
